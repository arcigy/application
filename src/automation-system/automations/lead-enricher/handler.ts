  ...
  try {
    for (const lead of uniqueDomainLeads) {
      ...
      try {
        ...
        let allEmailsRaw = [...new Set(scrapeResults.flatMap(r => r.emails))];
        ...
        if (allEmailsRaw.length === 0) {
          ...
          try {
            ...
            const validFallbacks = fallbackEmails.filter(e => !e.endsWith(".png") && !e.endsWith(".jpg") && !e.includes("example"));
